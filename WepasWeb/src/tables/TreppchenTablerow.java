...
public class TreppchenTablerow
{
	private static Log log = LogFactory.getLog(TreppchenTablerow.class);
	...
	public TreppchenTablerow(EnTipper user, EnWette wette, TipperBeanRemote tipperStateless, SpieltagBeanRemote spieltagStateless)
	{
		...
		{
			e.printStackTrace();
			log.error("TipperException Zeile 40" + e.getMessage());
		}
		...
		{
			e.printStackTrace();
			log.error("SpieltagException Zeile 58" + e.getMessage());
		}
